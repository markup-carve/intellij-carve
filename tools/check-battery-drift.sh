...
root="$(dirname "$here")"
local_file="$root/src/test/resources/battery/block-battery.json"
...
if ! diff -q "$local_file" "$remote_file" >/dev/null; then
  ...
  echo
  echo "Re-copy src/test/resources/battery/block-battery.json, then fix whatever the new shapes catch."
  exit 1
fi
