...
// Bundled from markup-carve/carve-js commit 3d95e948c1743f055e9d7445083ba702c0998592
// Built (UTC): 2026-08-06T16:18:36Z
...
var carve = (() => {
  ...
  function isBlankLine(line2) {
    return line2 !== void 0 && trimStructural(line2) === "";
  }
  var RE_BLOCKQUOTE = /^>(?: (.*)|)$/;
  ...
  var RE_RAW_FENCE = /^(`{3,}|~{3,}) *=([a-zA-Z][\w-]*)\s*$/;
  ...
  function nestedSubLexer(parent, source, startLineIndex, sourceLineMap) {
    const sub = subLexer(source, parent.parseOptions, parent.lineNumberOffset + startLineIndex, sourceLineMap ?? normalizedSourceLines(source).map((_line, i) => parent.lineNumber(startLineIndex + i)), parent.unclosedContainerKeys);
  ...
  function attachDocumentOffsets(sub, parent, startLineIndex) {
    ...
    for (let i = 0; i < sub.lines.length; i++) {
      ...
      const anchorsTo = (candidate) => {
        ...
        return parentLine2.endsWith(subLine) || parentLine2.endsWith(withoutSyntheticIndent(subLine));
      };
  ...
  function collectLinkDefs(lexer) {
    ...
    for (let idx = 0; idx < lexer.lines.length; idx++) {
      ...
      if (!fence) {
        ...
        } else if (raw.trim() !== "" && // A LINK REFERENCE DEFINITION at column 0 ends the item too, so it has
  ...
  function parseAdmonition(lexer) {
    ...
    const subLexer2 = nestedSubLexer(lexer, inner.map((line2) => line2.text).join("\n"), openLineIndex + 1);
  ...
  function isInvisibleLine(line2) {
    ...
    if (RE_COMMENT_LINE.test(l) || RE_LINK_DEF.test(l) || RE_FOOTNOTE_DEF.test(l))
      return true;
    return indentColumns(line2) === 0 && isBlockAttributeLine(l);
  }
  ...
  function parseList(lexer) {
    ...
    while (!lexer.eof()) {
      ...
      if (indentColumns(line2) !== baseIndent)
        break;
      const la = extractItemAttr(line2);
      const mline = la ? la.stripped : line2;
      ...
      const children = parseBlocks(mkSub([itemLead, ...leadLines].join("\n"), itemStartLineIndex, [
  ...
  function canonical(value) {
    ...
    if (value && typeof value === "object") {
      ...
      for (const key2 of Object.keys(value).sort()) {
        if (key2 === "pos" || key2 === "footnoteDefPos" || key2 === "srcByteLength")
          continue;
...
`.replace(/\ue000/g, " ");
  ...
  function refuseUnknownNodeTypes(node, path) {
    if (Array.isArray(node)) {
      node.forEach((item, index2) => refuseUnknownNodeTypes(item, `${path}[${index2}]`));
      return;
    }
    if (node === null || typeof node !== "object")
      return;
    const record = node;
    const type = record.type;
    if (typeof type === "string" && WIRE_FIELDS[type] === void 0) {
