...
// Bundled from markup-carve/carve-lsp commit 14320242aef83116d1d4096850779ad0ae628d18
// carve-js dependency npm 0.1.2
// Built (UTC): 2026-08-06T16:19:29Z
...
function promoteBlockImages(blocks, figuresOnly = false) {
  for (let i = 0; i < blocks.length; i++) {
    ...
    // not a figure target.
    !b.children[0].ref && // Strict column-0 rule: an image+caption forms a <figure> ONLY when the
...
// dist/inline-text.js
function smartPunctuationText(node) {
  if (typeof node !== "object" || node === null)
    return "";
  const candidate = node;
  if (candidate.type !== "smart_punctuation")
    return "";
  if (typeof candidate.glyph === "string")
...
function analyzeCarve(source) {
  ...
  for (const warning of djotMigrationWarnings(source)) {
    if (warning.category !== "carve-breakage")
      continue;
    diagnostics.push({
...
function plainText(nodes) {
  ...
  for (const node of nodes) {
    if (node.type === "text")
      ...
      out += node.value;
    else if (node.type === "literal_inline")
      out += node.content;
...
// dist/position.js
function codepointColumnToUtf16(lineText, column) {
  if (column <= 1)
    return 0;
  let codepointColumn = 1;
  let index2 = 0;
  while (index2 < lineText.length && codepointColumn < column) {
    index2 += (lineText.codePointAt(index2) ?? 0) > 65535 ? 2 : 1;
    codepointColumn++;
  }
  return index2 + Math.max(0, column - codepointColumn);
}
function utf16CharToCodepointColumn(lineText, character) {
  let codepointColumn = 1;
  let index2 = 0;
  while (index2 < lineText.length && index2 < character) {
    index2 += (lineText.codePointAt(index2) ?? 0) > 65535 ? 2 : 1;
    codepointColumn++;
  }
  return codepointColumn + Math.max(0, character - index2);
}
function sourceLines(source) {
  return source.replace(/\r\n?/g, "\n").split("\n");
}
var COLUMN_PROBE = "\u{1F600}x";
var CODEPOINT_PROBE_END_COLUMN = 3;
var cachedUnit;
function engineColumnUnit() {
  if (cachedUnit)
    return cachedUnit;
  try {
    const endColumn = parse2(COLUMN_PROBE).children[0]?.pos?.endColumn;
...
function astColumnToCharacter(lineText, column) {
  return engineColumnUnit() === "codepoint" ? codepointColumnToUtf16(lineText, column) : column - 1;
}
function characterToAstColumn(lineText, character) {
  ...
  return {
    ...
    range: {
      ...
      // line", as it did before.
        ...
        line,
        character: heading.pos.endColumn === void 0 ? 199 : astColumnToCharacter(sourceLines(source)[line] ?? "", heading.pos.endColumn)
      }
    }
  };
}
...
function hoverAt(source, position) {
  ...
  try {
    const hover = astHoverAt(resolve(parse2(source)), position, lines);
    if (hover)
      return hover;
  } catch {
  }
...
function astHoverAt(doc, position, lines) {
  const cursor = {
    line: position.line,
  ...
  for (const node of doc.children)
    collectBlock(matches, node, cursor);
...
connection.onRequest(import_node5.DocumentSymbolRequest.type, (params) => {
  const document = documents.get(params.textDocument.uri);
  return document ? analyzeCarve(document.getText()).symbols : [];
});
connection.onRequest(import_node5.HoverRequest.type, (params) => {
  const document = documents.get(params.textDocument.uri);
  return document ? hoverAt(document.getText(), params.position) : null;
});
connection.onRequest(import_node5.CodeActionRequest.type, (params) => {
  const document = documents.get(params.textDocument.uri);
  return document ? migrationCodeActions(params.textDocument.uri, document.getText(), params.context.diagnostics) : [];
});
connection.onRequest(import_node5.CompletionRequest.type, (params) => {
  const document = documents.get(params.textDocument.uri);
  return document ? completionAt(document.getText(), params.position) : [];
});
connection.onRequest(import_node5.DocumentFormattingRequest.type, (params) => {
  const document = documents.get(params.textDocument.uri);
  if (!document)
    return [];
  const text = document.getText();
...
connection.onRequest(import_node5.FoldingRangeRequest.type, (params) => {
  const document = documents.get(params.textDocument.uri);
  return document ? foldingRanges(document.getText()) : [];
});
connection.onRequest(import_node5.PrepareRenameRequest.type, (params) => {
  const document = documents.get(params.textDocument.uri);
  return document ? prepareRename(document.getText(), params.position) : null;
});
connection.onRequest(import_node5.RenameRequest.type, (params) => {
  const document = documents.get(params.textDocument.uri);
  return document ? renameEdits(params.textDocument.uri, document.getText(), params.position, params.newName) : null;
});
connection.onRequest(import_node5.CodeLensRequest.type, (params) => {
  const document = documents.get(params.textDocument.uri);
  return document ? codeLenses(document.getText()) : [];
});
connection.onRequest(import_node5.DefinitionRequest.type, (params) => {
  const document = documents.get(params.textDocument.uri);
  return document ? definitionAt(params.textDocument.uri, document.getText(), params.position) : null;
});
connection.onRequest(import_node5.ReferencesRequest.type, (params) => {
  const document = documents.get(params.textDocument.uri);
