...
- **The preview works offline and makes no outbound request** (#111). The preview page pulled six files from `cdn.jsdelivr.net` at render time - highlight.js and its two themes, Chart.js, MathJax and Mermaid - so with no network it silently lost code colours, charts, math and diagrams, and with a network every opened `.crv` file announced itself to a third party from inside the IDE. All six now ship in the plugin (5.2 MB; highlight.js 11.9.0 BSD-3-Clause, Chart.js 4.5.1 MIT, MathJax 3.2.2 Apache-2.0, Mermaid 11.17.2 MIT, each with its licence text) and are referenced from disk. Versions are the ones the CDN specifiers resolved to, pinned rather than upgraded.
- **A copy button on every code block** (#111). A 14px icon beside the language name, visible without hovering and solid under the pointer. It copies through the IDE rather than the browser clipboard, so the text lands where the rest of the IDE reads it and no clipboard permission is in the path; the button turns green or red for a moment so a copy that fails is never silent.
...
- **The preview is styled from the Carve design tokens** (#111). `tokens.css` was injected into the page and referenced zero times, while the page hardcoded a second palette (`#2c3e50`, `#3498db`, `#f4f4f4`, `#bdc3c7`) and restated all of it again in a parallel `body.dark` block. Two palettes in one document was the defect. Every colour now resolves through a `--carve-*` token and dark mode comes from `data-theme` on the root element - which an IDE theme switch now moves, so the whole page follows the theme instead of only the code colours doing so. Chart.js labels and grid lines take the same ink, so a chart is legible on a dark theme.
- **A code block is one flat surface** (#111). The heavy frame around code fences was not a border: the page painted the `<pre>`, and the highlight.js theme painted `pre code.hljs` a different shade with 1em of its own padding on top of it, leaving an inset rectangle. The theme now loads before the page's own rules and one rule zeroes that padding and background. Heading underlines are gone except a hairline under `h1`, and the language badge and header bar lost their boxes.
- **A heading's permalink appears on hover** (#111). The `headingPermalinks` extension puts an `<a class="permalink">` pilcrow after every heading and nothing styled it, so a document that is mostly headings ended every one of them in a coloured paragraph mark. It fades in when the heading is under the pointer or the link takes focus.
- **The preview, HTML export and language server run on a released engine, against a released spec** (#99). Both engine bundles are rebuilt from carve-js `37ed8904` - the published 0.1.5 - and the spec pin moves to carve `375e1f37`, the published 0.1.4. The two now agree exactly: 0 of 1538 corpus documents render differently, where the engine this release started from missed 87 of them. That pass moved only the engine inside the language server; the server itself follows below.
...
- **The bundled language server is a released one** (#110). The plugin shipped a server built from carve-lsp `ef1ca246` - twelve commits past `v0.1.3`, thirty-eight short of main, a revision no carve-lsp user could install. It is rebuilt from the published `v0.1.4` (`305ebbd8`), still linked against carve-js `37ed8904`, the published 0.1.5, so the two vendored engines stay one revision. `lsp/server.js` and `lsp/VERSION` name the release beside each commit now: a sha alone never said whether a pin was a release, which is how an unreleased server shipped in the first place, and `CarveBundleProvenanceTest` rejects a bundle built from an untagged commit.
